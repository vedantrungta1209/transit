...
export default function OtpScreen() {
  ...
  return (
    <View className="flex-1 bg-white px-6 justify-center">
      ...
      <Text className="text-gray-500 mb-8">Enter the 6-digit code sent to +91{phone}</Text>
      <View className="flex-row gap-3 mb-8 justify-center">
        {otp.map((digit, i) => (
          <TextInput
            key={i} ref={r => { if (r) refs.current[i] = r; }}
            value={digit} onChangeText={v => handleChange(v, i)}
            keyboardType="number-pad" maxLength={1}
            className="w-12 h-12 border-2 rounded-xl text-center text-xl font-bold border-gray-300 focus:border-blue-600"
          />
        ))}
      </View>
      <TouchableOpacity onPress={handleVerify} disabled={loading} className="bg-blue-600 rounded-xl py-4 items-center">
        <Text className="text-white font-semibold text-base">{loading ? 'Verifying...' : 'Verify'}</Text>
      </TouchableOpacity>
    </View>
  );
}
